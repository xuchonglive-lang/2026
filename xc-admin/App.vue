...
export default {
    ...
    init() {
      ...
      vk.userCenter.getMenu({
        success: (data) => {
          ...
          let { menus = [] } = data;
          // 合并去重
          menus = vk.pubfn.arr_concat(
            menus,
            config.sideBar.staticMenu,
            "menu_id"
          );
          // 排序
          menus.sort((a, b) => {
          ...
          if (
            JSON.stringify(menus) !== JSON.stringify(vk.getVuex("$app.navMenu"))
          ) {
          ...
          if (
            ...
            JSON.stringify(vk.getVuex("$app.menuList"))
          ) {
        ...
        vk.callFunction({
          ...
          success: (data) => {
            if (!data.info || data.info.appid !== systemInfo.appId) {
              vk.confirm(
                `您当前登录的应用【${systemInfo.appId}】未在已有应用列表中，是否需要去添加？`,
                "提示",
                "前往应用管理",
                "取消",
                (res) => {
  ...
  onLaunch: function (options) {
    let that = this;
    that.appOptions = options;
    // 接收页面token参数，用于实现自动登录
    that.vk.handleAutoLoginToken(options);
    if (config.debug) {
      console.log(
