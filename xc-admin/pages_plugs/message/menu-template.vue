...
<!-- https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx5aff5e86dbad0f92&redirect_uri=https%3A%2F%2Fwww.xuchong.online%2Findex.html%23%2Fpages%2Ftest%2Ftest&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect -->
			...
			<div class="menu clearfix">
				<div class="menu_bottom" v-for="(item, i) of menu.buttons" :key="i">
					...
					<div class="submenu" v-if="isSubMenuFlag == i">
						...
						<div class="menu_bottom menu_addicon" v-if="item.subButtons.length < 5"
		...
		methods: {
			...
			async mockMenuFun() {
				let data = await vk.callFunction({
									url: 'wxServer/menu/pub/getMenuData',
									title: '',
									data: {}
								});
								this.menu.buttons=data.menuData.buttons
								console.log(JSON.stringify(data.menuData, null, 5));
			...
			async saveFun() {
				...
				console.log("这是菜单",this.menu)
				
				let data = await vk.callFunction({
					url: 'wxServer/menu/pub/updateMenuData',
					title:'',
					data:{
						menu:this.menu
					},
