<template>
  <div class="page-body">
    ...
    <div class="header">
      ...
      <div class="header-item">
        <el-input
          class="search-input input-border-radius-4004"
          size="small"
          type="text"
          clearable
          placeholder="请输入名称搜索..."
    ...
    <div class="nav" v-loading="loading.nav">
      ...
      <div
        class="nav-list"
        v-for="(item, index) of data.navList"
        :key="item._id"
    ...
    <div class="main" v-loading="loading.main">
      ...
      <div
        ...
        v-for="(item, index) in data.content.rows"
        :key="item._id"
    ...
    <div class="footer" :class="selectedIds.length ? '' : 'no-selected'">
      ...
      <span class="footer-item" @click.stop="vk.pubfn.openForm('updateFileCategory', { item: { _id: selectedIds }, list: data.navList })"> 保存到组 </span>
      ...
      <el-pagination
        ...
        @size-change="
          (val) => {
            queryForm1.pageSize = val;
            getList();
          }
        "
        @current-change="
          (val) => {
            queryForm1.pageIndex = val;
            getList();
          }
        "
      ></el-pagination>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取文件列表
      getList() {
        let { queryForm1, selectedIds } = this;
        vk.callFunction({
      ...
      getNavList() {
        vk.callFunction({
          ...
          success: (data) => {
            this.data.navList = [{ _id: '', name: '全部' }, { _id: 'null', name: '未分组' }, ...data.rows];
          },
        });
      },
      // 查询指定分类下的文件列表
      ...
      deleteCategory(_id, index) {
        vk.callFunction({
          ...
          success: (data) => {
            this.data.navList.splice(index, 1);
          },
        });
      },
      // 删除
        ...
        vk.callFunction({
          ...
          success: (data) => {
            if (data.num > 0) {
              ...
              ids.map((id) => {
                let index = vk.pubfn.getListIndex(this.data.content.rows, '_id', id);
                if (index > -1) {
                  this.data.content.rows.splice(index, 1);
                }
              });
              this.selectedIds = [];
            }
      ...
      uploadFile() {
        let type = this.queryForm1.formData.type;
        let fileType;
        let extension = [];
        if (type === 'image') {
      ...
      selectAll() {
        if (this.selectedIds.length && this.data.content.rows.length == this.selectedIds.length) {
          ...
          this.data.content.rows.forEach((value, index, arr) => {
            if (!this.selectedIds.includes(value._id)) {
