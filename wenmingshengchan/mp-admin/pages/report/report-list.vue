  ...
  <view class="page-body">
    ...
    >
      <div v-if="detailDialog.data" style="max-height: 75vh; overflow-y: auto; overflow-x: hidden; padding-right: 10px;">
        ...
        <div style="background: #f3f4f6; border-radius: 8px; padding: 15px 20px 5px 20px; margin-bottom: 20px; font-size: 14px; color: #374151;">
          <el-row :gutter="20">
            ...
            <el-col :span="12" style="margin-bottom: 10px;">
              ...
              <el-tag size="mini" :type="detailDialog.data.status === 0 ? 'danger' : (detailDialog.data.status === 1 ? 'primary' : (detailDialog.data.status === 2 ? 'info' : 'warning'))">
                {{ detailDialog.data.status === 0 ? '待处理' : (detailDialog.data.status === 1 ? '处理中' : (detailDialog.data.status === 2 ? '已结案' : '已驳回')) }}
              </el-tag>
            </el-col>
            <el-col :span="12" style="margin-bottom: 10px;">
            ...
            <el-col :span="12" style="margin-bottom: 10px;">
              ...
              <span style="color: #111827;">{{ vk.pubfn.timeFormat(detailDialog.data._add_time, 'yyyy-MM-dd hh:mm:ss') }}</span>
            </el-col>
            <el-col :span="12" style="margin-bottom: 10px;">
        ...
        <el-timeline>
          <el-timeline-item :timestamp="vk.pubfn.timeFormat(detailDialog.data._add_time, 'yyyy-MM-dd hh:mm:ss')" placement="top" type="primary">
          ...
          <el-timeline-item 
            v-if="detailDialog.data.status !== 0 && detailDialog.data.reply_time" 
            :timestamp="vk.pubfn.timeFormat(detailDialog.data.reply_time, 'yyyy-MM-dd hh:mm:ss')" 
            placement="top" 
            :type="detailDialog.data.status === 2 ? 'success' : (detailDialog.data.status === 3 ? 'danger' : 'warning')"
          >
            <el-card shadow="hover">
              <h4 style="margin: 0 0 10px 0; color: #1f2937;">
                {{ detailDialog.data.status === 1 ? '工单正在处理中' : (detailDialog.data.status === 2 ? '工单已结案归档' : '工单已被驳回/作废') }}
              </h4>
              <p style="margin: 0; color: #6b7280; font-size: 14px;">
  ...
  data() {
    return {
      queryForm1: {
        formData: {},
        columns: [
          { key: "title", title: "问题标题", type: "text", mode: "%%" },
          { 
      ...
      table1: {
        ...
        columns: [
          { key: "title", title: "标题", type: "text", width: 160 },
          ...
          { key: "area_name", title: "作业区域", type: "text", width: 120 },
          { key: "point_name", title: "作业点位", type: "text", width: 120 },
          { key: "dept_name", title: "用户部门", type: "text", width: 120 },
          { 
            key: "status", title: "状态", type: "html", width: 100,
            formatter: (val) => {
              if (val === 0) return '<span style="color:#e11d48;font-weight:bold;">待处理</span>';
              if (val === 1) return '<span style="color:#0050cb;font-weight:bold;">处理中</span>';
              if (val === 2) return '<span style="color:#424656;font-weight:bold;">已结案</span>';
              if (val === 3) return '<span style="color:#cc4204;font-weight:bold;">已驳回</span>';
              return '';
            }
          },
      ...
      form1: {
        ...
        props: {
          ...
          columns: [
            ...
            { key: "reply_content", title: "回复内容", type: "textarea", autosize: { minRows: 4, maxRows: 10 }, maxlength: 500 },
            { key: "reply_images", title: "回复图片", type: "image", limit: 3 }
          ],
          rules: {
