<template>
	...
	<view class="addfankui">
		...
		<cu-custom bgColor="bg-gradual-orange" :isCustom="true">
			...
			<block slot="content">添加材料进场信息</block>
		</cu-custom>
		<!-- #endif -->
		<!-- 消息提示 -->
		...
		<u-form :model.sync="form" :rules="rules" ref="uForm" :errorType="errorType">
			<view class="cu-bar bg-white solid-bottom">
				<view class="action">
					<text class="cuIcon-titles text-blue"></text>添加材料进场信息
				</view>
			</view>

			<view class="block">
				<u-form-item :label-position="labelPosition" label="日期" prop="e_date">
					<cl-select :border="false" mode="date" placeholder="请选择材料进场日期" v-model="form.e_date"></cl-select>
				</u-form-item>

				<u-form-item :label-position="labelPosition" label="班次" prop="e_shift">
					<cl-radio-group v-model="form.e_shift">
				...
				<u-form-item  label-width="140" label="采购单位" prop="team_id">
				...
				<u-form-item label-width="140" label="进场井点" prop="rock_id">
					<cl-select :border="false" class="cl-select" v-model="form.rock_id" :options="rockshaftlist" 
				...
				<u-form-item label-width="140" :label-position="labelPosition" label="进场数量" prop="num">
					<cl-input :border="false" placeholder="请填写进场材料数量(单位:t)" v-model="form.num"></cl-input>
				</u-form-item>
	...
	export default {
		...
		methods: {
			...
			async initFormData(id) {
				...
				that.entryofmaterial_id = res.item._id
			...
			async getRockshaftList() {
				...
				that.rock_id = that.rockshaftlist[0].value
