<template>
	<view>
		...
		<!--有内容时开始-->
			...
			<view class="order" v-for="(item, index) in entryofmaterialList" :key="item._id" @tap="toDetail(item._id)">
              ...
              {{item.team_name}}
				...
				<view class="bottom">
					...
					<view class="showZerenren">
						<image class="zerenren"
							:src="(item.avatar||'')"  mode="aspectFill"></image>{{item.realName}}
					</view>
					<view class="duty"><text class="text">{{item.e_date.slice(0,10)}} {{item.shift}}</text></view>
				</view>
...
<script>
	...
	export default {
		...
		methods:{
			toDetail(id){
				vk.navigateTo(`/pages/list/detail/entryofmaterial/entryofmaterialDetail?id=${id}`);
			}
		}
	}
</script>
