			...
			<view class="body" @click.stop @touchmove.stop.prevent>
				<picker-view :value="valueArr" @change="change" class="picker-view" @pickstart="pickstart"
					...
					<picker-view-column v-if="!reset && params.year">
						<view class="column1" v-for="(item, index) in years" :key="index">
							{{ item }}
							<text class="text" v-if="showTimeTag">年</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && params.month">
						<view class="column1" v-for="(item, index) in months" :key="index">
							{{ formatNumber(item) }}
							<text class="text" v-if="showTimeTag">月</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && params.day">
						<view class="column1" v-for="(item, index) in days" :key="index">
							{{ formatNumber(item) }}
							<text class="text" v-if="showTimeTag">日</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && params.hour">
						<view class="column1" v-for="(item, index) in hours" :key="index">
							{{ formatNumber(item) }}
							<text class="text" v-if="showTimeTag">时</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && params.minute">
						<view class="column1" v-for="(item, index) in minutes" :key="index">
							{{ formatNumber(item) }}
							<text class="text" v-if="showTimeTag">分</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && params.second">
						<view class="column1" v-for="(item, index) in seconds" :key="index">
							{{ formatNumber(item) }}
...
<script>
	export default {
		...
		methods: {
			...
			setMonths() {
				//范围选择开始月份
				let bm = this.beginMonthDay.split('-')[0]
				//范围选择结束月份
				let em = this.endMonthDay.split('-')[0]
			...
			setDays() {
				...
				let em = this.endMonthDay.split('-')[0]
				//范围选择开始日
				let bd = this.beginMonthDay.split('-')[1] || 1
				//范围选择结束日
				let ed = this.endMonthDay.split('-')[1] || totalDays
				
				if (bm == em) {
					this.days = this.generateArray(bd, ed)
				}
				else if (bm == this.month) {
					this.days = this.generateArray(bd, totalDays)
				}
				else if (em == this.month) {
