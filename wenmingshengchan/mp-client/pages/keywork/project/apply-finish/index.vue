<template>
  <view class="page-container industrial-bg">
    ...
    <view class="custom-nav-ui">
      ...
      <text class="nav-title font-headline font-extrabold text-industrial-primary">{{ mode === 'close' ? '申请验收结案' : '提交进度反馈' }}</text>
    ...
    <view class="form-wrapper">
      ...
      <view class="page-banner">
        <text class="banner-en font-bold">{{ mode === 'close' ? 'SUBMIT CLOSURE REQUEST' : 'SUBMIT PROCESS FEEDBACK' }}</text>
      ...
      <u-form :model="form" ref="uForm" class="industrial-form">
        ...
        <view class="glass-card input-card">
          <view class="field-header">
            <view class="field-icon"><text class="material-symbols-outlined">edit_square</text></view>
        ...
        <view class="glass-card input-card">
          <view class="field-header">
            <view class="field-icon"><text class="material-symbols-outlined">add_photo_alternate</text></view>
            <view class="field-title-group">
              <text class="field-title font-headline font-bold">佐证照片</text>
              <text class="field-subtitle">ATTACHMENTS</text>
            </view>
            <view class="limit-badge">最多4张</view>
          </view>
          <view class="upload-container">
            <view class="image-grid">
              <view class="image-item" v-for="(img, idx) in form.attachment_imgs" :key="idx">
                <image :src="img" mode="aspectFill" class="preview-img" @click="previewImage(idx)"></image>
                <view class="delete-btn" @click.stop="deleteImage(idx)">
                  <text class="material-symbols-outlined">close</text>
                </view>
              </view>
              <view class="upload-add-btn" @click="chooseImage" v-if="form.attachment_imgs.length < 4">
      ...
      <view class="submit-footer">
        <button class="btn-primary-ui4 active-press" :class="{'is-loading': loading}" @click="submit" :disabled="loading">
          <text class="material-symbols-outlined" v-if="!loading">send</text>
          <text class="btn-text">{{ loading ? '正在提交同步中...' : (mode === 'close' ? '确认提交验收' : '确认提交反馈') }}</text>
        </button>
      </view>
    </view>
    <u-toast ref="uToast" />
...
.cancel-btn {
  font-size: 32rpx;
  color: #e85545; /* Red color matching the screenshot */
  font-weight: 800;
  letter-spacing: 2rpx;
  display: flex;
  align-items: center;
...
.nav-placeholder {
  width: 64rpx; /* Balance for the title */
}
